...
export const MODELS = {
  ...
  opus: "claude-sonnet-4-5" // Redirect Opus to Sonnet
} as const;
...
<config_files>
- Configuration files (.config.ts, .config.js, tailwind.config.ts, vite.config.ts, drizzle.config.ts, tsconfig.json, etc.) must be created/placed in the PROJECT ROOT DIRECTORY, NOT in client/ or server/ subdirectories
- Always use the root directory for config files unless explicitly told otherwise
</config_files>
</file_operations>
...
CRITICAL FILE CREATION INSTRUCTION:
When user asks to "create", "make", "do", "сделай файл", "создай файл", "сделай", or anything similar - YOU MUST:
1. IMMEDIATELY CALL write_file tool (do NOT think, plan, or talk about it)
2. Use the write_file tool with:
   - path: the requested filename
   - content: the file content (or just the filename if user wants it empty)
3. THEN respond to user about what you created
4. Do NOT describe creating files - ACTUALLY USE THE write_file TOOL

Example:
User: "make a file test.txt"
You: [CALL write_file TOOL immediately with path="test.txt", content="test.txt"]
Then: "Создал файл test.txt"
