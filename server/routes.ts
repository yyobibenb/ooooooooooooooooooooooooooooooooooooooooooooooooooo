...
export async function registerRoutes(
  ...
  app.post(api.chat.send.path, isAuthenticated, async (req, res) => {
    try {
      ...
      let response;
      try {
        response = await streamChatResponse(chatMessages, res, {
          taskType: "chat",
          projectContext,
          conversationId,
        ...
        const errorMsg = (streamErr as any)?.message || "Failed to generate response";
        res.write(`data: ${JSON.stringify({ done: true, error: errorMsg })}\n\n`);
      ...
      const fullResponse = typeof response === 'string' ? response : (response?.content || "");
      const responseMetadata = response && typeof response === 'object' && 'metadata' in response ? (response as any).metadata : undefined;
