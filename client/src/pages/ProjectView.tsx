...
import { 
  ...
  Globe, Terminal, Play, Pencil, Check, X, Square, FolderOpen
} from "lucide-react";
...
export default function ProjectView() {
  ...
  const [activeTab, setActiveTab] = useState<"code" | "preview" | "console" | "chat">("code");
  ...
  const handleRun = async () => {
    ...
    // Listen for agent/orchestrator events if we decide to expose them via SSE
    // Or we can just ensure the console output includes these messages from the backend
  ...
  return (
    <div className="h-screen bg-gradient-to-b from-blue-50 via-slate-50 to-white overflow-hidden p-0 md:p-4">
      {isMobile ? (
        <div className="h-full flex flex-col relative">
          <header className="p-4 border-b border-white/40 bg-white/40 backdrop-blur-md flex items-center justify-between">
          ...
          <main className="flex-1 overflow-hidden pb-24">
            {activeTab === "chat" && (
              <div className="h-full px-4 pt-4 pb-0 flex flex-col">
                <ChatPanel projectId={projectId} currentFile={activeFile} />
              </div>
            )}
            {activeTab === "code" && (
                ...
                {activeFile ? (
                  <>
                    <div className="px-4 py-2 border-b border-white/30 flex items-center justify-between bg-white/20">
                      <span className="text-xs text-gray-600 font-medium truncate">{activeFile.name}</span>
                      <button onClick={handleSave} className="text-blue-500 text-xs font-semibold">Save</button>
                    </div>
                  ...
                  <div className="h-full flex flex-col items-center justify-center p-8 text-center text-gray-400 gap-4">
                    <Code2 className="w-12 h-12" />
                    <p>Select a file from the explorer to edit</p>
                    <button onClick={() => setActiveTab("console")} className="text-blue-500 text-sm underline">Open Explorer</button>
                  </div>
                )}
              </div>
            )}
              ...
              <div className="h-full flex flex-col">
                <div className="flex-1 bg-white">
                  {previewPort && isRunning ? (
                    <iframe src={`http://localhost:${previewPort}`} className="w-full h-full border-0" title="Preview" />
                  ) : (
                    <div className="h-full flex flex-col items-center justify-center text-gray-400 gap-2">
                      <Globe className="w-12 h-12" />
                      <p>Run project to see preview</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            {activeTab === "console" && (
              <div className="h-full flex flex-col">
                <div className="flex-1 flex flex-col overflow-hidden">
                   <div className="flex-1 bg-white/40 overflow-auto">
                      <FileExplorer projectId={projectId} activeFileId={activeFile?.id ?? null} onSelectFile={(file) => { setActiveFile(file); setActiveTab("code"); }} />
                   </div>
                   <div ref={consoleRef} className="h-1/3 p-4 font-mono text-xs bg-gray-900 text-gray-100 overflow-auto border-t border-white/20">
                      {consoleOutput.length === 0 ? "Console logs will appear here..." : consoleOutput.map((l, i) => <pre key={i} className="whitespace-pre-wrap">{l}</pre>)}
                   </div>
                </div>
              </div>
            )}
          ...
          <nav className="mobile-nav-glass mb-4">
            <button 
              onClick={() => setActiveTab("chat")}
              className={cn("mobile-nav-item", activeTab === "chat" ? "mobile-nav-item-active" : "mobile-nav-item-inactive")}
            >
              <Terminal className="w-5 h-5" />
              {activeTab === "chat" && <span>Chat</span>}
            </button>
            <button 
              onClick={() => setActiveTab("code")}
              className={cn("mobile-nav-item", activeTab === "code" ? "mobile-nav-item-active" : "mobile-nav-item-inactive")}
            >
            ...
            <button 
              ...
              className={cn("mobile-nav-item", activeTab === "preview" ? "mobile-nav-item-active" : "mobile-nav-item-inactive")}
            >
              <Globe className="w-5 h-5" />
              {activeTab === "preview" && <span>Live</span>}
            </button>
            <button 
              onClick={() => setActiveTab("console")}
              className={cn("mobile-nav-item", activeTab === "console" ? "mobile-nav-item-active" : "mobile-nav-item-inactive")}
            >
              <FolderOpen className="w-5 h-5" />
              {activeTab === "console" && <span>Files</span>}
            </button>
          </nav>
        </div>
      ) : (
              ...
              >
                {consoleOutput.length === 0 ? (
                  ...
                  consoleOutput.map((line, idx) => {
                    // Check if the line looks like a typical command prompt
                    const isNixPrompt = line.trim().startsWith("[nixos@replit:~]$") || line.trim().startsWith("➜");
                    return (
